                    ...
                    <div className="p-4 pb-3">
                        <div className="campaign-setup-section">
                            ...
                            <div className="campaign-setup-grid grid gap-3 sm:grid-cols-2">
                                <div className="relative flex flex-col gap-2" ref={channelMenuRef}>
                                    ...
                                    {channelMenuOpen && (
                                        <div className="absolute left-0 top-full mt-2 w-full bg-white border border-gray-200 rounded-2xl shadow-xl p-2 z-30">
                                ...
                                <div className="relative flex flex-col gap-2" ref={accountMenuRef}>
                                    ...
                                    >
                                        <span className="flex items-center gap-2 truncate">
                                            <img src={channelIconSrc.Google} className="w-4 h-4" alt="Google Ads" />
                                            <span className="truncate">{activeGoogleAccount ? activeGoogleAccount.name : 'No account connected'}</span>
                                        </span>
                                        <ChevronDownIcon className={`w-3 h-3 text-gray-400 transition-transform ${accountMenuOpen ? 'rotate-180' : ''}`} />
                                    </button>
                                    {accountMenuOpen && (
                                        <div className="absolute left-0 top-full mt-2 w-full bg-white border border-gray-200 rounded-2xl shadow-xl p-3 z-30 max-h-72 overflow-auto">
                                            <div className="text-sm font-medium text-gray-800 mb-2">Select account</div>
                                ...
                                <div className="relative flex flex-col gap-2" ref={dropdownRef}>
                                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Markets</span>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setChannelMenuOpen(false);
                                    ...
                                    >
                                        <span className="flex items-center gap-2">
                                            ...
                                            <span>{marketItems.length === 0 ? 'Select markets' : `${marketItems.length} selected`}</span>
                                        </span>
                                        <ChevronDownIcon className={`w-3 h-3 text-gray-400 transition-transform ${showMarkets ? 'rotate-180' : ''}`} />
                                    </button>
                                    <div className="flex flex-wrap gap-2 min-h-[42px] rounded-xl border border-dashed border-gray-200 bg-gray-50 px-3 py-2">
                                        {marketItems.length === 0 ? (
                                            <span className="text-xs text-gray-400">Use “Select markets” to add markets or clusters.</span>
                                        ) : (
                                            marketItems.map((c, i) => (
                                                <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white border border-gray-200 text-xs">
                                ...
                                <div className="relative flex flex-col gap-2" ref={campaignTypeMenuRef}>
                                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Campaign types</span>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setChannelMenuOpen(false);
                                            setAccountMenuOpen(false);
                                            setShowMarkets(false);
                                            setCampaignTypeMenuOpen(v => !v);
                                        }}
                                        className="flex items-center justify-between w-full px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:border-gray-300 transition-colors"
                                    >
                                        <span>{selectedCampaignTypes.length === 0 ? 'Select campaign types' : `${selectedCampaignTypes.length} selected`}</span>
                                        <ChevronDownIcon className={`w-3 h-3 text-gray-400 transition-transform ${campaignTypeMenuOpen ? 'rotate-180' : ''}`} />
                                    </button>
                                    <div className="flex flex-wrap gap-2 min-h-[42px] rounded-xl border border-dashed border-gray-200 bg-gray-50 px-3 py-2">
                                        {selectedCampaignTypes.length === 0 ? (
                                            <span className="text-xs text-gray-400">Choose one or more campaign types.</span>
                                        ) : (
                                            selectedCampaignTypes.map(type => (
                                                <span key={type} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white border border-gray-200 text-xs">
                                                    {type}
                                    ...
                                    {campaignTypeMenuOpen && (
                                        <div className="absolute left-0 top-full mt-2 w-72 bg-white border border-gray-200 rounded-2xl shadow-xl p-3 z-30">
                                            ...
                                            <div className="mt-3 flex justify-end gap-2">
                                                ...
                                                <button type="button" onClick={() => setCampaignTypeMenuOpen(false)} className="text-xs px-3 py-1.5 rounded-full bg-black text-white">Done</button>
                                            </div>
                                        </div>
                                    )}
                                </div>
